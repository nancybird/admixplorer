...
apply_threshold_selection <- function(improvements, method, fst, cv) {
  ...
  # Load thresholds from package data
  data("thresholds_config", package = "admixcraft", envir = environment())
  ...
  cat(sprintf("Thresholds: k1tok2(k=2): %.2f, k1tok2(k=3): %.2f, k2tok3: %.2f, k3tok4: %.2f\n",
  ...
  if ("k1_to_k2" %in% names(scaled_improvements)) {
    if (scaled_improvements$k1_to_k2 > threshold_k1_to_k2_for_k2) {
      recommended_k <- "2"
      cat(sprintf("k1tok2 improvement (%.4f) > threshold for k=2 (%.2f): Consider k=2\n",
      ...
      if ("k2_to_k3" %in% names(scaled_improvements)) {
        if (scaled_improvements$k2_to_k3 > threshold_k2_to_k3) {
          # For k=3, we need BOTH conditions: higher k1tok2 AND good k2tok3
          if (scaled_improvements$k1_to_k2 > threshold_k1_to_k2_for_k3) {
            recommended_k <- "3"
            cat(sprintf("k2tok3 improvement (%.4f) > threshold (%.2f) AND k1tok2 (%.4f) > k=3 threshold (%.2f): Consider k=3\n",
            ...
            if ("k3_to_k4" %in% names(scaled_improvements)) {
              if (scaled_improvements$k3_to_k4 > threshold_k3_to_k4) {
                ...
                cat(sprintf("k3tok4 improvement (%.4f) > threshold (%.2f): Consider k=4 or higher\n",
                            scaled_improvements$k3_to_k4, threshold_k3_to_k4))
              } else {
                cat(sprintf("k3tok4 improvement (%.4f) less than or equal to  threshold (%.2f): Stop at k=3\n",
                            scaled_improvements$k3_to_k4, threshold_k3_to_k4))
              }
            }
          } else {
            cat(sprintf("k2tok3 improvement (%.4f) > threshold (%.2f) BUT k1tok2 (%.4f) less than or equal to  k=3 threshold (%.2f): Stop at k=2\n",
                        scaled_improvements$k2_to_k3, threshold_k2_to_k3,
                        scaled_improvements$k1_to_k2, threshold_k1_to_k2_for_k3))
          }
        } else {
          cat(sprintf("k2tok3 improvement (%.4f) less than or equal to  threshold (%.2f): Stop at k=2\n",
                      scaled_improvements$k2_to_k3, threshold_k2_to_k3))
        }
      }
    } else {
      cat(sprintf("k1tok2 improvement (%.4f) less than or equal to  threshold (%.2f): Stop at k=1\n",
                  scaled_improvements$k1_to_k2, threshold_k1_to_k2_for_k2))
    }
  }
