...
generate_final_output <- function(all_mcmc_results, recommended_k, original_data, pop.vec,
                                  dates_original, sample_age_est, outfile, method, fst,
                                  thresholds_info) {
  ...
  # Enhanced summary header with FST info
  summary_header <- paste0(
    "# LIKELIHOOD-BASED K SELECTION RESULTS (", toupper(method), ")\n",
    "# Recommended K: ", recommended_k, "\n",
    "# Method: ", method, "\n",
    "# Input FST: ", fst, " (scaling factor: ", round(thresholds_info$fst_scaling$scaling_factor, 3), ")\n",
  ...
  write.table(combined_output, output.outfile, row.names = FALSE, col.names = TRUE,
