...
test_that("apply_threshold_selection handles missing k=2 results", {
  # Only k=1 results available (no k=2)
  mock_results <- list(
    "1" = list(
      result = list(
        final_log_likelihood_best_sampling_ages = c(-100),
  ...
  mock_improvements <- list(
    completed_ks = c("1"),  # ADD THIS LINE - specify which k values succeeded
    improvements = list()   # No improvements since only k=1 exists
  )
  ...
  # Should fall back to likelihood-based decision and recommend k=1
  expect_equal(result$recommended_k, "1")
})
